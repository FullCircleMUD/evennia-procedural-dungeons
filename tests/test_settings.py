...
"""Minimal Django settings for evennia-procedural-dungeons unit tests.
...
LOG_DIR = os.path.join(tempfile.gettempdir(), "evennia_procedural_dungeons_test_logs")
os.makedirs(LOG_DIR, exist_ok=True)

# Library under test
INSTALLED_APPS = list(INSTALLED_APPS) + [  # noqa: F405
    "evennia_procedural_dungeons",
]

# One database. The library owns no tables — it wires exits between rooms the
# consumer's world already holds, and both are in the game database.
DATABASES = {
    "default": {
        "ENGINE": "django.db.backends.sqlite3",
        "NAME": ":memory:",
        "TEST": {
            "NAME": "file:evennia_procedural_dungeons_test_default?mode=memory&cache=shared"
        },
    },
}
